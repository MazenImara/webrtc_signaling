...
var isInitiator;
var room = 1;//window.location.hash.substring(1);
if (!room) {
  room = window.location.hash = 1;//randomToken();
}
...
addCamBtn.addEventListener('click', addCamFun);
function addCamFun(argument) {
  socket.emit('cmd', 'addCam');
}



var removeCamBtn = document.getElementById('removeCamBtn');
removeCamBtn.addEventListener('click', removeCamFun);
function removeCamFun(argument) {
  socket.emit('cmd', 'removeCam');
}
